...
For production environments, it is crucial to gracefully shut down your application to avoid dropping in-flight requests. You can use the built-in `GracefulShutdown` plugin for this.

```typescript
import { Shokupan, GracefulShutdown } from 'shokupan';
...
You can then use the `@OnStop` decorator in your controllers or the `onStop` hook to clean up resources like database connections:
